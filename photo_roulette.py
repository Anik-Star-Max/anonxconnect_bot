...
class PhotoRoulette:
    ...
    def add_photo(self, user_id: int, photo_file_id: str, caption: str = "") -> bool:
    ...
    def get_random_photo(self, current_user_id: int) -> tuple:
    ...
    def rate_photo(self, rater_user_id: int, photo_owner_id: int, is_like: bool = True) -> tuple:
        """Rate a photo (like or dislike)"""
        rater_str = str(rater_user_id)
        owner_str = str(photo_owner_id)

        # Check if already rated
        if owner_str in self.photos_data["likes"].get(rater_str, []):
    ...
    def get_user_photo_stats(self, user_id: int) -> dict:
        """Get user's photo statistics"""
        user_str = str(user_id)

        if user_str not in self.photos_data["photos"]:
            return {"message": "No photos found for this user."}
    ...
    def remove_user_photo(self, user_id: int) -> bool:
    ...
    def get_top_photos(self, limit: int = 10) -> list:
